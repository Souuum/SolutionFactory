...
export function createUserMailer({ to, token }: ResetPasswordMailer) {
  // In production, set APP_ORIGIN to your production server origin
  const origin = process.env.APP_ORIGIN || process.env.BLITZ_DEV_SERVER_ORIGIN
  const resetUrl = `${origin}/auth/signup?token=${token}`
  ...
  const msg = {
    ...
    to,
    subject: "Vos instructions de création de compte",
    text: "message envoyé",
    template: "create",
    context: {
      url: resetUrl,
